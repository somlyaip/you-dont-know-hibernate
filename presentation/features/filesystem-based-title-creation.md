...
- Top‑level slides (Markdown or HTML, not inside a directory):
 ...
 - Vertical slide stacks (a numeric‑prefixed directory with ordered children):
  ...
  - Additionally, a single stack header is rendered inside the stack wrapper as `<h2 class="stack-header">{directory title}</h2>`. It is hidden on the first (title) slide and visible on subsequent slides in the stack.
  ...
  - Note: The stack header exists in addition to the dedicated title slide, to provide context on slides after the first.
  
Exceptions:
 - HTML slides that contain a <meme-slide ...> element:
  - Top‑level: no heading is injected.
  - Inside a vertical stack: no heading is injected (neither H2 nor H3). The directory intro slide still appears at the start of the stack, and the stack header is shown on subsequent slides (it remains hidden on the first/title slide).
...
- Source file: `013-every-problem-has-a-solution-1.html`
  - Injected into HTML (if top‑level and not the first): `<h2>every-problem-has-a-solution-1</h2>` will be added.
  - In a vertical stack: a separate first title slide shows `<h2>{directory name}</h2>`, and each child slide gets only `<h3>{child filename}</h3>` injected at the top of its content.
  - If the file contains a `<meme-slide>` element, then no `<h2>`/`<h3>` is injected for that slide. In all meme-slide cases, the `<meme-slide>` tag will have `title="..."` set from the filename-based title.
